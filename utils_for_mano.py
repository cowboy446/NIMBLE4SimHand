...
Modified for MANO hand model by:
    Zhang Rong, Aug 21 2025
'''
...
# Only this is for MANO hand model
...
# ------------- These are for NIMBLE hand model -------------
...
def posevec_2axisang_nimble(pose_3d, rest_pose_3d):
    ...
    for i in range(len(used_children)):
        ...
        if parent == -1:
            axis_ang_local_list[:, i, :] = axis_ang_list[:, i, :]
            continue
...
def posevec_2axisang_mano(pose_3d, rest_pose_3d):
    ...
    for i in range(len(used_children)):
        child_idx = used_children[i]
        parent = JOINT_PARENT_ID_DICT_MANO[child_idx]
        if parent == -1:
            axis_ang_local_list[:, i, :] = axis_ang_list[:, i, :]
            continue
        if parent in exclude_children:
